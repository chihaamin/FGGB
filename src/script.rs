...
unsafe extern "C" fn call_on_message<I: ScriptHandler>(
    _script_ptr: *mut bind::_FridaScript,
    message: *const i8,
    _data: &bind::_GBytes,
...
impl<'a> Script<'a> {
    pub(crate) fn from_raw(script_ptr: *mut bind::_FridaScript) -> Script<'a> {
    ...
    pub fn load(&self) -> Result<()> {
        ...
        unsafe { bind::frida_script_load_sync(self.script_ptr, null_mut(), &mut error) };
    ...
    pub fn unload(&self) -> Result<()> {
        let mut error: *mut bind::GError = std::ptr::null_mut();
        unsafe { bind::frida_script_unload_sync(self.script_ptr, null_mut(), &mut error) };
    ...
    pub fn handle_message<I: ScriptHandler + 'static>(&mut self, handler: I) -> Result<()> {
        ...
        unsafe {
            ...
            bind::_frida_g_signal_connect_data(
    ...
    pub fn post<S: AsRef<str>>(&self, message: S, data: Option<&[u8]>) -> Result<()> {
        ...
        unsafe {
            let g_data = if let Some(data) = data {
                bind::_frida_g_bytes_new(data.as_ptr() as _, data.len() as _)
            } else {
                std::ptr::null_mut()
            };
            bind::frida_script_post(self.script_ptr as _, message.as_ptr() as _, g_data);
            bind::_frida_g_bytes_unref(g_data);
        }
...
impl<'a> Drop for Script<'a> {
    fn drop(&mut self) {
        unsafe { bind::frida_unref(self.script_ptr as _) }
    }
}
...
impl From<ScriptRuntime> for bind::FridaScriptRuntime {
    fn from(runtime: ScriptRuntime) -> Self {
        match runtime {
            ScriptRuntime::Default => bind::FridaScriptRuntime_FRIDA_SCRIPT_RUNTIME_DEFAULT,
            ScriptRuntime::QJS => bind::FridaScriptRuntime_FRIDA_SCRIPT_RUNTIME_QJS,
            ScriptRuntime::V8 => bind::FridaScriptRuntime_FRIDA_SCRIPT_RUNTIME_V8,
        }
    }
}
...
pub struct ScriptOption {
    ptr: *mut bind::FridaScriptOptions,
}

impl ScriptOption {
    /// Create a new set of script options.
    pub fn new() -> Self {
        let ptr = unsafe { bind::frida_script_options_new() };
        Self { ptr }
    }

    /// Get the name of the script.
    pub fn get_name(&self) -> &'static str {
        let name = unsafe { CStr::from_ptr(bind::frida_script_options_get_name(self.ptr)) };
        name.to_str().unwrap_or_default()
    }

    /// Set the name of the script.
    pub fn set_name(self, name: &str) -> Self {
        unsafe { bind::frida_script_options_set_name(self.ptr, name.as_ptr() as _) };
        self
    }

    /// Set the runtime of the script.
    pub fn set_runtime(self, runtime: ScriptRuntime) -> Self {
        unsafe { bind::frida_script_options_set_runtime(self.ptr, runtime.into()) };
        self
    }

    pub(crate) fn as_mut_ptr(&mut self) -> *mut bind::FridaScriptOptions {
        self.ptr
    }
}
...
impl Drop for ScriptOption {
    fn drop(&mut self) {
        unsafe {
            bind::_frida_g_clear_object(&mut self.ptr as *mut *mut bind::_FridaScriptOptions as _)
        }
    }
}
