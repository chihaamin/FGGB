...
pub struct Process<'a> {
    process_ptr: *mut bind::_FridaProcess,
    phantom: PhantomData<&'a bind::_FridaProcess>,
}

impl<'a> Process<'a> {
    pub(crate) fn from_raw(process_ptr: *mut bind::_FridaProcess) -> Process<'a> {
        Process {
            process_ptr,
            phantom: PhantomData,
        }
    ...
    pub fn get_name(&self) -> &str {
        let process_name =
            unsafe { CStr::from_ptr(bind::frida_process_get_name(self.process_ptr) as _) };
    ...
    pub fn get_pid(&self) -> u32 {
        unsafe { bind::frida_process_get_pid(self.process_ptr) }
    }
}

impl<'a> Drop for Process<'a> {
    fn drop(&mut self) {
        unsafe { bind::frida_unref(self.process_ptr as _) }
    }
}
...
/// Process Spawn Optionsbind::
pub struct SpawnOptions<'a> {
    pub(crate) options_ptr: *mut bind::FridaSpawnOptions,
    phantom: PhantomData<&'a bind::FridaSpawnOptions>,
}

impl<'a> SpawnOptions<'a> {
    pub(crate) fn from_raw(options_ptr: *mut bind::FridaSpawnOptions) -> Self {
        Self {
            options_ptr,
            phantom: PhantomData,
        }
    ...
    pub fn new() -> Self {
        Self::from_raw(unsafe { bind::frida_spawn_options_new() })
    }
        ...
        L: IntoIterator<Item = S>,
    {
        ...
        unsafe {
            bind::frida_spawn_options_set_argv(
                self.options_ptr,
                arg_ptrs.as_mut_ptr(),
    ...
    pub fn cwd<S: AsRef<CStr>>(self, cwd: S) -> Self {
        unsafe {
            bind::frida_spawn_options_set_cwd(self.options_ptr, cwd.as_ref().as_ptr() as *mut _);
        }
        self
    }
        ...
        M: IntoIterator<Item = (K, V)>,
    {
        ...
        unsafe {
            bind::frida_spawn_options_set_env(
                self.options_ptr,
                env_ptrs.as_mut_ptr(),
        ...
        M: IntoIterator<Item = (K, V)>,
    {
        ...
        unsafe {
            bind::frida_spawn_options_set_envp(
                self.options_ptr,
    ...
    pub fn stdio(self, stdio: SpawnStdio) -> Self {
        unsafe { bind::frida_spawn_options_set_stdio(self.options_ptr, stdio as _) }
        self
    }
}
